...
- 🎯 Entry points to execute your program using `python -m <wealth>` or `$ wealth` with basic CLI argument parsing.
...
---
# wealth Flask Application

Awesome wealth created by rpozdeev
...
```bash
git clone https://github.com/rpozdeev/wealth wealth
cd wealth
make install
```
...
```bash
wealth create-db   # run once
wealth populate-db  # run once (optional)
wealth add-user -u admin -p 1234  # ads a user
wealth run
```
